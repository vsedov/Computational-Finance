...
print(
    ...
    trees(S0, X1, T, r, sigma, "call", n=3),
)
print(
    f"European put option price (Binomial Tree) => {X1}:",
    trees(S0, X1, T, r, sigma, "put", n=3),
)
# print(
...
print(
    ...
    trees(S0, X1, T, r, sigma, "call", n=3, american=True),
)
print(
    f"American put option price (Binomial Tree) => {X1}:",
    trees(S0, X1, T, r, sigma, "put", n=3, american=True),
)


# Find Theta
def theta_gama_delta_tree_eu_american_option(S0, X1, T, r, sigma, american):
    n = 3
    delta_t = T / n
    u = np.exp(sigma * np.sqrt(delta_t))
    d = 1 / u
    ...
    for i in range(n + 1):
        for j in range(i + 1):
            tree[j, i] = S0 * (d**j) * (u ** (i - j))
    option_values = np.maximum(tree[:, -1] - X1, 0)
    for i in range(n - 1, -1, -1):
        for j in range(i + 1):
            option_values[j] = np.exp(-r * delta_t) * (
                p * option_values[j] + (1 - p) * option_values[j + 1]
            ...
            if american:
                option_values[j] = np.maximum(option_values[j], tree[j, i] - X1)
    # return option_values[0]
    return f"Theta: {option_values[0] - option_values[1]}\nGamma: {(option_values[0] - option_values[2]) - (option_values[1] - option_values[2])}\nDelta: {(option_values[0] - option_values[1]) / (S0 * u - S0 * d)}"


print(
    theta_gama_delta_tree_eu_american_option(S0, X1, T, r, sigma, american=True)
)
